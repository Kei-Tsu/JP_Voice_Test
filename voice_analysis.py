    ...
    try:
        ...
        if recording_source == "microphone" and intimacy_level in ["intimate", "very_intimate"]:
            ...
            if avg_drop_rate < 0.15:
                clarity_level = "良好"
                advice = "親密な会話として語尾までしっかり発話できています！マイク録音でもこの明瞭さは素晴らしいです。"
                score = min(100, int((1 - avg_drop_rate * 0.8) * 100))
            elif avg_drop_rate < 0.3:
                clarity_level = "普通"
                advice = "親密な会話レベルとしては普通です。マイク録音を考慮すると、実際の会話ではもう少し語尾を意識すると良いでしょう。"
                score = int(75 - (avg_drop_rate - 0.15) * 80)
            elif avg_drop_rate < 0.45:
                clarity_level = "やや弱い"
                advice = "文末の音量がやや弱めです。親密な会話でも、文末を1音上げるイメージで話してみましょう。"
                score = int(60 - (avg_drop_rate - 0.3) * 80)
            else:
                clarity_level = "要改善"
                advice = "語尾の音量が大きく低下しています。親密な関係でも結論が正しく伝わるよう意識してみましょう。"
